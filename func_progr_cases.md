...
- [DolphinDB 函数化编程案例](#dolphindb-函数化编程案例)
	- [1. 数据导入](#1-数据导入)
		- [1.1. 整型时间转化为 TIME 格式并导入](#11-整型时间转化为-time-格式并导入)
		- [1.2. 有纳秒时间戳的文本导入](#12-有纳秒时间戳的文本导入)
	- [2. Lambda 表达式](#2-lambda-表达式)
	- [3. 高阶函数使用案例](#3-高阶函数使用案例)
		- [3.1. cross 使用案例](#31-cross-使用案例)
			- [3.1.1. 将两个向量或矩阵，两两组合作为参数来调用函数](#311-将两个向量或矩阵两两组合作为参数来调用函数)
			- [3.1.2. 计算股票两两之间的相关性](#312-计算股票两两之间的相关性)
		- [3.2. each 使用案例](#32-each-使用案例)
			- [3.2.1. 获取数据表各个列的 NULL 值个数](#321-获取数据表各个列的-null-值个数)
			- [3.2.2. 去除表中存在 NULL 值的行](#322-去除表中存在-null-值的行)
			- [3.2.3. 按行处理与按列处理性能比较案例](#323-按行处理与按列处理性能比较案例)
			- [3.2.4. 判断两张表内容是否相同](#324-判断两张表内容是否相同)
		- [3.3. loop 使用案例](#33-loop-使用案例)
			- [3.3.1. loop 与 each 的区别](#331-loop-与-each-的区别)
			- [3.3.2. 导入多个文件](#332-导入多个文件)
		- [3.4. moving/rolling 使用案例](#34-movingrolling-使用案例)
			- [3.4.1. moving 案例](#341-moving-案例)
			- [3.4.2. moving(sum) 和 msum 性能差距](#342-movingsum-和-msum-性能差距)
		- [3.5. eachPre 使用案例](#35-eachpre-使用案例)
		- [3.6. byRow 使用案例](#36-byrow-使用案例)
		- [3.7. segmentby 使用案例](#37-segmentby-使用案例)
		- [3.8. pivot 使用案例](#38-pivot-使用案例)
		- [3.9. contextby 使用案例](#39-contextby-使用案例)
		- [3.10. call/unifiedCall 使用案例](#310-callunifiedcall-使用案例)
		- [3.11. accumulate 使用案例](#311-accumulate-使用案例)
		- [3.12. window 使用案例](#312-window-使用案例)
		- [3.13. reduce 使用案例](#313-reduce-使用案例)
	- [4. 部分应用案例](#4-部分应用案例)
		- [4.1. 提交带有参数的作业](#41-提交带有参数的作业)
		- [4.2. 获取集群其它节点作业信息](#42-获取集群其它节点作业信息)
		- [4.3. 带“状态”的流计算消息处理函数](#43-带状态的流计算消息处理函数)
	- [5. 金融场景相关案例](#5-金融场景相关案例)
		- [5.1. 使用 map reduce，对 tick 数据降精度](#51-使用-map-reduce对-tick-数据降精度)
		- [5.2. 数据回放和高频因子计算](#52-数据回放和高频因子计算)
		- [5.3. 基于字典的计算](#53-基于字典的计算)
	- [6. 机器学习相关案例](#6-机器学习相关案例)
		- [6.1. ols 残差](#61-ols-残差)
	- [7. 总结](#7-总结)
...
在DolphinDB 中可以使用高阶函数 `cross` 或 `pcross` 计算协方差矩阵：
...
定义好表结构后，如需获取对应的数据，可前往 [Tushare](https://tushare.pro/document/1?doc_id=39) 平台注册账户，获取 TOKEN，并参考 [案例脚本](./script/getTushareDailyLine.py) 进行数据导入操作。本案例使用 DolphinDB 的 [Python API](../api/python/py.dita) 获取数据，用户也可参考 Tushare 的说明文档使用其它语言或库。本例使用 2008 年到 2017 年的日线行情进行说明。
...
DolphinDB 还提供了内置函数 `segment` 用于在SQL语句中进行分组。与 segmentby 不同，它只返回分组信息，而不对分组进行计算。
