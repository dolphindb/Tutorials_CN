...
- [概念](#概念)
- [Array Vector 支持的函数和操作](#array-vector-支持的函数和操作)
	- [Array Vector 的创建](#array-vector-的创建)
	- [Array Vector 的基础操作](#array-vector-的基础操作)
	- [Array Vector 的计算](#array-vector-的计算)
	- [API 写入](#api-写入)
- [Array Vector 在 level2 快照数据中的应用](#array-vector-在-level2-快照数据中的应用)
	- [快照数据的存储](#快照数据的存储)
	- [快照数据分组时保留明细](#快照数据分组时保留明细)
	- [基于快照数据的高频因子计算](#基于快照数据的高频因子计算)
- [总结](#总结)
- [附件](#附件)
...
（1）通过 [array](https://www.dolphindb.cn/cn/help/FunctionsandCommands/FunctionReferences/a/array.html?highlight=array) 或 [bigarray](https://www.dolphindb.cn/cn/help/FunctionsandCommands/FunctionReferences/b/bigarray.html) 函数定义空的数组向量，并通过 [append!](https://www.dolphindb.cn/cn/help/FunctionsandCommands/FunctionReferences/a/append!.html) 添加数据
...
（2）通过 [fixedLengthArrayVector](https://www.dolphindb.cn/cn/help/FunctionsandCommands/FunctionReferences/f/fixedLengthArrayVector.html) 将多个向量/元组/矩阵或表拼接成数组向量。
...
（3）通过 [arrayVector](https://www.dolphindb.cn/cn/help/FunctionsandCommands/FunctionReferences/a/arrayVector.html) 将单个向量拆分成数组向量。
...
通过 [setColumnarTuple!](https://www.dolphindb.cn/cn/help/FunctionsandCommands/FunctionReferences/s/setColumnarTuple!.html) 函数将一个普通元组转换成列式元组。
...
（2）通过 [fixedLengthArrayVector](https://www.dolphindb.cn/cn/help/FunctionsandCommands/FunctionReferences/f/fixedLengthArrayVector.html) 将表中多列拼成一列。
...
（4）设置 [loadText](https://www.dolphindb.cn/cn/help/FunctionsandCommands/FunctionReferences/l/loadText.html?highlight=loadtext) 的 *schema* 和 *arrayDelimiter*，从文本文件中读取含 Fast Array Vector 列的表。

用 [saveText](https://www.dolphindb.cn/cn/help/FunctionsandCommands/CommandsReferences/s/saveText.html?highlight=savetext) 存储数据时，Fast Array Vector 的列会自动存储为如下格式：Array Vector 内部用 arrayDelimiter 隔开。
...
用户可以通过函数（[row](https://www.dolphindb.cn/cn/help/FunctionsandCommands/FunctionReferences/r/row.html?highlight=row)、[at](https://www.dolphindb.cn/cn/help/FunctionsandCommands/FunctionReferences/a/at.html)）的方式访问 Array Vector 的行列；也可以通过下标（“x[index]”）的方式访问 Array Vector 的元素。
...
- 通过 [row](https://www.dolphindb.cn/cn/help/FunctionsandCommands/FunctionReferences/r/row.html?highlight=row) 函数读取 Array Vector 中的一行。【只能访问一行，返回一个向量】
...
- 通过 [`tableInsert`](https://www.dolphindb.cn/cn/help/FunctionsandCommands/FunctionReferences/t/tableInsert.html?highlight=tableinsert)  、 `append!` 等方法向含有 Array Vector 列的表中插入数据
...
（1）通过 [flatten](https://www.dolphindb.cn/cn/help/FunctionsandCommands/FunctionReferences/f/flatten.html?highlight=flatten) 函数可以将 Array Vector 展开成一维向量 。
...
（2）通过 [matrix](https://www.dolphindb.cn/cn/help/FunctionsandCommands/FunctionReferences/m/matrix.html?highlight=matrix) 函数可以将等长的 Array Vector 转化为矩阵 。
...
（1）Array Vector 支持 [行计算系列（row 系列）](https://www.dolphindb.cn/cn/help/FunctionsandCommands/SeriesOfFunctions/rowFunctions.html?highlight=row)。
...
[rowAlign(left, right, how)](https://www.dolphindb.cn/cn/help/FunctionsandCommands/FunctionReferences/r/rowAlign.html)：实现 left 和 right 的数据对齐。输入参数 *left*、*right* 是数组向量，表示需要对齐的数据；*how* 是字符串，表述对齐的方式；最终返回一个长度为 2 的元组，分别表示对齐后数据在原数据中的索引。

<img src="images/Array_Vector/Array_Vector_1.png">

[rowAt(X, Y)](https://www.dolphindb.cn/cn/help/FunctionsandCommands/FunctionReferences/r/rowAt.html)：实现按行从 X 中取出 Y 索引的元素。输入参数 X 是矩阵或数组向量；当 Y 是和 X 行数相等的向量时，返回一个与 Y 长度相同的向量；当 Y 是和 X 行数相等的数组向量时，返回一个与 Y 维度相同的数组向量。
...
（2）Fast Array Vector 支持调用高阶函数 [byRow](https://www.dolphindb.cn/cn/help/Functionalprogramming/TemplateFunctions/byRow.html)，对数组向量的每行元素进行计算。
...
（3）Array Vector 支持调用高阶函数 [each](https://www.dolphindb.cn/cn/help/Functionalprogramming/TemplateFunctions/each.html?highlight=each) 和 [loop](https://www.dolphindb.cn/cn/help/Functionalprogramming/TemplateFunctions/loopPloop.html?highlight=loop)，对 Array Vector 的每行元素进行计算。
...
（示例中，是通过 upload 方法把表数据 table 上传到 DolphinDB 的内存表 myTable。C++ API 的更多操作可以查看教程：[C++ API使用教程](https://gitee.com/dolphindb/api-cplusplus)）
...
（示例中，是通过 `tableInsert` 方法把表数据 tb 上传到 DolphinDB 的维度表 loadTable ('dfs://testDB','test')。Java API 的更多操作可以查看教程：[Java API使用教程](https://gitee.com/dolphindb/api-java/blob/master/README_CN.md#/dolphindb/api-java/blob/master/../release200/README_CN.md)）
...
（示例中，是通过 `table` 方法把数据框 df 上传到 DolphinDB 的内存表 myTable。Python API 的更多操作可以查看教程：[Python API使用教程](https://gitee.com/dolphindb/api_python3/blob/master/README_CN.md)）
...
根据《[DolphinDB 处理 Level 2 行情数据实例](https://gitee.com/dolphindb/Tutorials_CN/blob/master/Level-2_stock_data_processing.md#314-十档净委买增额)》 中 3.1.4 章节，实现的指标公式如下：

<img src="images/Array_Vector/Array_Vector_15.png">

其中 ![images/Array_Vector/level10.png](images/Array_Vector/level10.png) 表示 t 时刻的委买增额； $bid_{i,t}$ 表示 t 时刻快照数据的第 i 档买方报价；指示函数 $I$ 含义如下：
