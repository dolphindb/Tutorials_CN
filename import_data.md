...
在传统的数据库系统，分区是针对Table定义的，就是同一个Database里的每个Table都可以有自己的分区定义；
而DolphinDB的分区是针对Database定义的，也就是说同一个Database下的Table只能使用同一种分区机制，这也意味着如果两张表要使用不同的分区机制，那么它们是不能放在一个Database下的。
