...
默认情况下，流数据表把所有数据保存在内存中。随着流数据持续写入，系统可能会出现内存不足的情况。为了避免这个问题，我们可以将流数据持久化到磁盘。持久化的数据支持重订阅，当订阅指定数据下标时，下标的计算包含持久化的数据。流数据持久化另一个重要的功能是流数据的备份和恢复，当节点出现异常重启时，持久化的数据在启用持久化时会载入到流数据表。
