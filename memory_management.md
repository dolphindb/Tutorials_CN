...
(mem().allocatedBytes - mem().freeBytes).sum()
```
...
`mem()`函数可以显示整个节点的内存分配和占用情况。该函数输出4列，其中列blockSize表示分配的内存块大小，freeSize表示剩余的内存块大小，通过sum(mem().allocatedBytes - mem().freeBytes) 得到节点所使用的总的内存大小。
